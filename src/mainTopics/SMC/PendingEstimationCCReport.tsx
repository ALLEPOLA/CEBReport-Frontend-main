...
interface PendingEstimationCCItem {
...
interface PendingEstimationCCSummary {
    fromDate: string;
    toDate: string;
    costCtr: string;
    jobType: string;
...
interface ColumnDef {
    ...
    label: string;
    isDate?: boolean;
    getValue: (it: PendingEstimationCCItem) => string;
}

const columns: ColumnDef[] = [
    { key: "ApplicationType", label: "Application Type", getValue: (it) => it.ApplicationType ?? "" },
    { key: "ApplicationSubType", label: "Application Sub Type", getValue: (it) => it.ApplicationSubType ?? "" },
    { key: "ApplicationId", label: "Application ID", getValue: (it) => it.ApplicationId ?? "" },
    { key: "ApplicationNo", label: "Application No", getValue: (it) => it.ApplicationNo ?? "" },
    { key: "Name", label: "Applicant Name", getValue: (it) => it.Name ?? "" },
    { key: "CusAddress", label: "Applicant Address", getValue: (it) => it.CusAddress ?? "" },
    { key: "SubmitDate", label: "Submit Date", isDate: true, getValue: (it) => it.SubmitDate ?? "" },
    { key: "TariffCode", label: "Tariff", getValue: (it) => it.TariffCode ?? "" },
    { key: "Phase", label: "Phase", getValue: (it) => it.Phase ?? "" },
    { key: "ConnectionType", label: "Connection Type", getValue: (it) => it.ConnectionType ?? "" },
];

const cellValue = (it: PendingEstimationCCItem, col: ColumnDef) => {
    const raw = col.getValue(it);
    return col.isDate ? formatDate(raw) : raw;
};

/* ────── Table Modal Component ────── */
const PendingEstimationCCTable: React.FC<{
    data: PendingEstimationCCItem[];
    summary: PendingEstimationCCSummary | null;
    fromDate: string;
    toDate: string;
    costCenter: string;
    ...
    jobTypeLabel: string;
    onClose: () => void;
}> = ({ data, summary, fromDate, toDate, costCenter, departmentName, jobTypeSelected, jobTypeLabel, onClose }) => {
    const maroon = "text-[#7A0000]";
    const fromLabel = formatDate(fromDate);
    const toLabel = formatDate(toDate);
    const reportTitle = jobTypeSelected
        ? `Cost center wise pending ${jobTypeLabel} estimation report From ${fromLabel} To ${toLabel}`
        : `Cost center wise pending all estimation report From ${fromLabel} To ${toLabel}`;
    const costCenterLine = `Cost Center : ${costCenter}${departmentName ? " - " + departmentName : ""}`;

    const sortedData = [...data]; // already ordered by backend (application_type, application_sub_type, application_no)
    ...
    const downloadCSV = () => {
        ...
        a.href = url;
        a.download = `PendingEstimationCC_${costCenter}_${fromDate}_${toDate}.csv`;
        a.click();
    ...
    const printPDF = () => {
        ...
        sortedData.forEach((it, i) => {
            const cells = columns.map((c) => `<td>${cellValue(it, c) || ""}</td>`).join("");
            rowsHTML += `
...
<head>
  <style>
    @media print {
      ...
      .costcenter { margin: 0 8px 12px; text-align:center; font-weight:600; color:#374151; font-size:10px; }
...
<body>
  ...
  <div class="costcenter">${costCenterLine} &nbsp;&nbsp; Records: ${totalRecords}</div>
  <table>
    <thead>
      <tr>
        <th>No</th>
...
/* ────── MAIN COMPONENT ────── */
const PendingEstimationCCReport: React.FC = () => {
    const { user } = useUser();
    ...
    const [jobType, setJobType] = useState(ALL_JOB_TYPES_VALUE);
    const [jobTypesLoading, setJobTypesLoading] = useState(true);

    const [selectedDept, setSelectedDept] = useState<Department | null>(null);
    const [reportData, setReportData] = useState<PendingEstimationCCItem[]>([]);
    const [reportSummary, setReportSummary] = useState<PendingEstimationCCSummary | null>(null);
    ...
    useEffect(() => {
        const fetchJobTypes = async () => {
            ...
            try {
                ...
                setJobTypes(types);
                // Left at ALL_JOB_TYPES_VALUE ("") by default - unlike CompletedJobsCC, a
                // blank jobType here is a valid, meaningful choice ("all job types").
            } catch (e: any) {
    ...
    const fetchReport = async (dept: Department) => {
        ...
        try {
            ...
            const url = `/misapi/api/pendingestimationcc/report?fromDate=${fromParam}&toDate=${toParam}&costCtr=${dept.DeptId}${
            ...
            const items: PendingEstimationCCItem[] = json.data || [];
            if (items.length === 0) {
    ...
    const jobTypeSelected = jobType !== ALL_JOB_TYPES_VALUE;
    ...
    return (
        ...
        >
            <h2 className={`text-xl font-bold mb-4 ${maroon}`}>
                Cost Center Wise Pending Estimation
            </h2>
            ...
            <div className="bg-gray-50 p-4 rounded-lg mb-4 border border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            <option value={ALL_JOB_TYPES_VALUE}>
                                {jobTypesLoading ? "Loading..." : "All Types"}
                            </option>
                            {jobTypes.map((jt) => (
                    ...
                    <div className="relative bg-white w-full max-w-[95vw] sm:max-w-4xl md:max-w-6xl lg:max-w-7xl rounded-2xl shadow-2xl overflow-hidden">
                        {reportLoading && (
                            <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center gap-4">
                                ...
                                <p className="text-sm text-gray-600">Fetching pending estimation records from server</p>
                            </div>
                        )}
                        {!reportLoading && reportData.length > 0 && (
                            <PendingEstimationCCTable
                                data={reportData}
...
export default PendingEstimationCCReport;