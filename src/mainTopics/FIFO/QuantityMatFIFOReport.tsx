...
/* ────── MAIN COMPONENT ────── */
const QuantityMatFIFOReport: React.FC = () => {
	const {user} = useUser();
	...
	/* ── Material code (optional filter) ── */
	const [matCode, setMatCode] = useState("");

	/* ── Report state ── */
	const [reportData, setReportData] = useState<QuantityMatFIFOItem[]>([]);
	...
	const fetchReport = async () => {
		...
		try {
			...
			// matCode is optional - leave the query param blank to get all material codes.
			const matCodeParam = encodeURIComponent(matCode.trim());
			const url = `/misapi/api/quantitymatfifo/report?costCtr=${costCtrParam}&whCode=${whParam}&matCode=${matCodeParam}`;
			...
			const items: QuantityMatFIFOItem[] = json.data || [];
	...
	const grandTotalQtyOnHand = reportData.reduce((s, r) => s + (r.QtyOnHand || 0), 0);
	const grandTotalValue = reportData.reduce((s, r) => s + (r.Value || 0), 0);
	const cctName = reportData.find((r) => r.CctName)?.CctName || selectedDept?.DeptName || "";
	const costCtrDisplay = selectedDept?.DeptId || "";
	const matCodeDisplay = matCode.trim() ? matCode.trim() : "ALL";
	const reportTitle = "Quantity On Hand Report (Material Wise)";
	...
	const downloadCSV = () => {
		...
		sortedData.forEach((it, i) => {
			...
			rows.push(
				[
					...
					csvEscape(formatNumber(it.Value)),
				].join(",")
			);
		});

		rows.push(
			`Total,,,,,,${csvEscape(formatNumber(grandTotalQtyOnHand))},,${csvEscape(
		...
		a.href = url;
		a.download = `QuantityMatFIFO_${selectedWarehouse}_${matCode.trim() || "ALL"}.csv`;
		a.click();
	...
	/* ────── PDF print ──────
	   ...
	   used here does NOTHING in that window. Every visual rule must be written as
	   inline `style="..."` or defined in the <style> block below.
	*/
	const printPDF = () => {
		if (reportData.length === 0) return;

		let rows = "";
		const cellBase = "padding:6px 8px; border:1px solid #d1d5db; font-size:8.5px;";
		...
		sortedData.forEach((it, i) => {
          ...
          <tr style="background:${rowBg};">
            ...
            <td style="${cellBase} text-align:left;">${it.MatNm || ""}</td>
            <td style="${cellBase} text-align:center; ${mono}">${it.WrhCd || ""}</td>
            <td style="${cellBase} text-align:left;">${it.MajUom || ""}</td>
            <td style="${cellBase} text-align:center;">${it.GrdCd || ""}</td>
            ...
            <td style="${cellBase} text-align:right; ${mono}">${formatNumber(it.Value)}</td>
          </tr>`;
		});
...
<head>
  <style>
    @media print {
      ...
      .sign-row { margin-top:30px; display:flex; justify-content:space-between; padding:0 15px; font-size:9px; }
      @page {
...
<body>
  ...
  <div class="info">
    ...
    <div><strong>Material Code:</strong> ${matCodeDisplay}</div>
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8.5px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        ...
        <th style="padding:6px 8px; width:11%; text-align:center;">Material Code</th>
        <th style="padding:6px 8px; width:19%; text-align:left;">Material Name</th>
        <th style="padding:6px 8px; width:10%; text-align:center;">Warehouse Code</th>
        <th style="padding:6px 8px; width:10%; text-align:left;">Major UOM</th>
        <th style="padding:6px 8px; width:9%; text-align:center;">Grade Code</th>
        ...
        <th style="padding:6px 8px; width:13%; text-align:right;">Value</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
    <tfoot>
      <tr>
        ...
        <td style="text-align:right; padding:6px 8px; border:1px solid #d1d5db; font-family:monospace;">${formatNumber(grandTotalQtyOnHand)}</td>
        <td style="padding:6px 8px; border:1px solid #d1d5db;"></td>
        <td style="text-align:right; padding:6px 8px; border:1px solid #d1d5db; font-family:monospace;">${formatNumber(grandTotalValue)}</td>
      </tr>
    </tfoot>
  </table>
  ...
  <div class="sign-row">
    ...
    <div>Certified Correct: ____________________</div>
	...
	return (
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans">
			<div className="flex justify-between items-center mb-4">
				<h2 className={`text-xl font-bold ${maroon}`}>Quantity On Hand Report (Material Wise)</h2>
			</div>
				...
				<div className="flex flex-col md:flex-row flex-wrap gap-3 items-end justify-between">
					<div className="flex flex-col">
						...
						<input
							...
							className="pl-3 pr-3 py-1.5 w-full md:w-56 rounded-md border border-gray-300 bg-white focus:outline-none focus:ring-2 focus:ring-[#7A0000] transition text-sm"
						/>
					</div>
					...
					<div className="relative bg-white w-[95vw] sm:w-[90vw] md:w-[85vw] lg:w-[80vw] xl:w-[75vw] max-w-7xl rounded-2xl shadow-2xl border border-gray-200 overflow-hidden mt-20 md:mt-32 lg:mt-40 lg:ml-64 mx-auto print:relative print:w-full print:max-w-none print:rounded-none print:shadow-none print:border-none print:overflow-visible">
						{reportLoading && (
							<div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center gap-4">
								...
								<p className="text-sm text-gray-600">Fetching quantity on hand data from server</p>
							</div>
						)}
						{!reportLoading && reportData.length > 0 && (
							<div className="p-2 md:p-2 max-h-[80vh] overflow-y-auto print:p-0 print:max-h-none print:overflow-visible print:mt-10 print:ml-12">
								...
								<div className="flex justify-between text-sm mb-3 ml-5 mr-12">
									...
									<div>
										<span className="font-bold">Material Code:</span> {matCodeDisplay}
									</div>
										...
										<table className="w-full text-xs border-collapse">
											<thead className={`${maroonGrad} text-white`}>
												<tr>
													...
													<th className="px-4 py-2 border border-gray-300 text-center" style={{width: "11%"}}>Material Code</th>
													<th className="px-4 py-2 border border-gray-300 text-left" style={{width: "19%"}}>Material Name</th>
													<th className="px-4 py-2 border border-gray-300 text-center" style={{width: "10%"}}>Warehouse Code</th>
													<th className="px-4 py-2 border border-gray-300 text-left" style={{width: "10%"}}>Major UOM</th>
													<th className="px-4 py-2 border border-gray-300 text-center" style={{width: "9%"}}>Grade Code</th>
													...
													<th className="px-4 py-2 border border-gray-300 text-right" style={{width: "13%"}}>Value</th>
												</tr>
											</thead>
											<tbody>
												{sortedData.map((it, i) => (
													<tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
														...
														<td className="px-4 py-2 text-left border-r border-gray-300">{it.MatNm || ""}</td>
														<td className="px-4 py-2 font-mono text-center border-r border-gray-300">{it.WrhCd || ""}</td>
														<td className="px-4 py-2 text-left border-r border-gray-300">{it.MajUom || ""}</td>
														<td className="px-4 py-2 text-center border-r border-gray-300">{it.GrdCd || ""}</td>
														...
														<td className="px-4 py-2 text-right font-mono border-r border-gray-300">{formatNumber(it.Value)}</td>
													</tr>
												))}
											</tbody>
											<tfoot>
												<tr className="bg-[#d3d3d3] font-bold">
													...
													<td className="px-4 py-2 text-right font-mono border border-gray-300">{formatNumber(grandTotalQtyOnHand)}</td>
													<td className="px-4 py-2 border border-gray-300"></td>
													<td className="px-4 py-2 text-right font-mono border border-gray-300">{formatNumber(grandTotalValue)}</td>
												</tr>
											</tfoot>
										</table>
								...
								<div className="flex justify-between text-sm mt-10 mb-6 ml-5 mr-12 print:hidden">
									<div>Prepared By: ____________________</div>
									<div>Certified Correct: ____________________</div>
									<div>Checked By: ____________________</div>
									<div>Accountant: ____________________</div>
...
export default QuantityMatFIFOReport;