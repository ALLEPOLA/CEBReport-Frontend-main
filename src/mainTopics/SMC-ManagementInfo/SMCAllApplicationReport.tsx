...
/* ────── Types ────── */
interface SMCAllApplicationItem {
    DeptId: string | null;
    IdNo: string | null;
    ...
    BillNo: string | null;
    JobAllocatedDate: string | null;
    ContractorName: string | null;
    JobFinishedDate: string | null;
    Area: string | null;
...
interface SMCAllApplicationSummary {
    fromDate: string;
    toDate: string;
    compId: string;
    totalRecords: number;
...
const SMCAllApplicationTable: React.FC<{
    data: SMCAllApplicationItem[];
    summary: SMCAllApplicationSummary | null;
    fromDate: string;
    toDate: string;
    compId: string;
    onClose: () => void;
    ...
    const areaLabel = data[0]?.CompNm || compId;
    const reportTitle = `Area wise SMC All Details - Project Assigned date From ${fromLabel} To ${toLabel}`;
    ...
    const totalEstimatedCost = summary?.totalPivAmount ?? sortedData.reduce((s, it) => s + (it.PivAmount || 0), 0);
    ...
    const printPDF = () => {
        ...
        <tr style="background:#d3d3d3; font-weight:bold;">
          ...
          <td style="text-align:right;">${formatAmount(totalEstimatedCost)}</td>
        </tr>
      `;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/90 print:static print:inset-auto print:p-0 print:bg-white">
            <div className="relative bg-white w-[97vw] max-w-[1800px] rounded-2xl shadow-2xl border border-gray-200 overflow-hidden mt-20 md:mt-24 lg:mt-28 lg:ml-64 mx-auto print:relative print:w-full print:max-w-none print:rounded-none print:shadow-none print:border-none print:overflow-visible">
                <div className="p-4 max-h-[85vh] overflow-y-auto print:p-0 print:max-h-none print:overflow-visible">
                    ...
                    <div className="text-sm mb-4 px-2 text-gray-700">
                            ...
                            <table className="w-full text-xs border-collapse">
                                <thead>
                                    <tr className="bg-[#7A0000] text-white sticky top-0">
                                        ...
                                        <th className="px-2 py-1.5 border border-gray-300 font-bold">Account No</th>
                                ...
                                <tbody>
                                    {sortedData.map((it, i) => (
                                        <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                                            ...
                                            <td className="px-2 py-1 border-r border-gray-300 font-mono">{it.AccountNo}</td>
                                ...
                                <tfoot>
                                    <tr className="bg-[#d3d3d3] font-bold sticky bottom-0">
                                        ...
                                        <td className="px-2 py-1.5 border border-gray-300 text-right font-mono">
                                            {formatAmount(totalEstimatedCost)}
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
...
/* ────── MAIN COMPONENT ────── */
const SMCAllApplicationReport: React.FC = () => {
    const { user } = useUser();
    ...
    const [reportData, setReportData] = useState<SMCAllApplicationItem[]>([]);
    const [reportSummary, setReportSummary] = useState<SMCAllApplicationSummary | null>(null);
    ...
    const handleViewReport = async (company: { id: string; name: string }) => {
        ...
        try {
            const url = `/misapi/api/smcallapplication/report?fromDate=${fromDate}&toDate=${toDate}&compId=${encodeURIComponent(
                company.id
            )}`;
            ...
            const items: SMCAllApplicationItem[] = json.data || [];
            if (items.length === 0) {
    ...
    return (
        ...
        >
            <h2 className={`text-xl font-bold mb-4 ${maroon}`}>SMC All Application Report</h2>
                    ...
                    <div className="relative bg-white w-full max-w-[95vw] sm:max-w-4xl md:max-w-6xl lg:max-w-7xl rounded-2xl shadow-2xl overflow-hidden">
                        {reportLoading && (
                            <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center gap-4">
                                ...
                                <p className="text-sm text-gray-600">Fetching SMC application details from server</p>
                            </div>
                        )}
                        {!reportLoading && reportData.length > 0 && (
                            <SMCAllApplicationTable
                                data={reportData}
...
export default SMCAllApplicationReport;