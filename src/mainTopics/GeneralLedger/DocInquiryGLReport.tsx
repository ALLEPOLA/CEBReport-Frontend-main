...
/* ────── MAIN COMPONENT ────── */
const DocInquiryGlReport: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<DocInquiryGlItem[]>([]);
	...
	const fetchReport = async (dept: Department) => {
		...
		try {
			...
			const url = `/misapi/api/docinquirygl/report/${fromDate}/${toDate}/${costCtrParam}`;
			...
			const items: DocInquiryGlItem[] = json.data || [];
	...
	const downloadCSV = () => {
		...
		const titleRows = [
			`Cost center wise general ledger inquiry report from ${fromDate} To ${toDate}`,
		...
		sortedData.forEach((it, i) => {
			rows.push(
				[
					...
					csvEscape(it.TrfDept),
				].join(",")
			);
		});

		rows.push(
			`Total,,,,,${csvEscape(formatNumber(grandTotalDrAmt))},${csvEscape(
				formatNumber(grandTotalCrAmt)
			)},${csvEscape(formatNumber(grandTotalTrxVal))},,`
		);
		...
		a.href = url;
		a.download = `DocInquiryGl_${fromDate}_${toDate}.csv`;
		a.click();
...
<body>
  <div class="title">Cost center wise general ledger inquiry report from ${fromDate} To ${toDate}</div>
  <div class="info">
  ...
  <table style="width:100%; border-collapse:collapse; font-size:8px; border:1px solid #d1d5db;">
    <thead>
      <tr style="background:linear-gradient(to right,#7A0000,#A52A2A); color:white;">
        <th style="padding:5px 6px; width:5%;">No</th>
        <th style="padding:5px 6px; width:10%;">Document Profile</th>
        <th style="padding:5px 6px; width:10%;">Transaction Date</th>
        <th style="padding:5px 6px; width:10%;">Document No</th>
        <th style="padding:5px 6px; width:9%;">GL Code</th>
        ...
        <th style="padding:5px 6px; width:11%; text-align:right;">Transaction Amount</th>
        <th style="padding:5px 6px; width:15%;">Description</th>
        <th style="padding:5px 6px; width:10%;">TRF Dept ID</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
    <tfoot>
		...
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans">
			<div className="flex justify-between items-center mb-4">
				<h2 className={`text-xl font-bold ${maroon}`}>
					Document Inquiry GL (All Transaction)
				</h2>
			</div>
...
export default DocInquiryGlReport;